...
async function handleDustLost(
  ...
  let newDustLost = await DustLost.create({
    ...
    aid: await getID(),
    timestamp: block.timestamp,
    totalBalanceBeforeDestroy: totalBalanceBeforeDestroy,
  });
  await newDustLost.save();
...
export const handleTransfer = async (
  ...
  let aId = await getID();
  const transfer = new Transfer(`${blockNum}-${aId}`);
...
export const handleBalanceSet = async (
  ...
  let aId = await getID();
  const balanceSet = new BalanceSet(`${blockNum}-${aId}`);
...
export const handleDeposit = async (
  ...
  let aId = await getID();
  const deposit = new Deposit(`${blockNum}-${aId}`);
...
export const handleReserved = async (
  ...
  let aId = await getID();
  const reserved = new Reserved(`${blockNum}-${aId}`);
...
export const handleUnreserved = async (
  ...
  // Create the new Reserved entity
  let aId = await getID();
  const unreserved = new Unreserved(`${blockNum}-${aId}`);
...
export const handleWithdraw = async (
  ...
  let aId = await getID();
  const withdraw = new Withdraw(`${blockNum}-${aId}`);
...
export const handleSlash = async (
  ...
  // Create the new Withdraw entity
  let aId = await getID();
  const slash = new Slash(`${blockNum}-${aId}`);
...
export const handleReservRepatriated = async (
  ...
  // Create the new Reserved entity
  let aId = await getID();
  const reservRepatriated = new ReservRepatriated(`${blockNum}-${aId}`);
